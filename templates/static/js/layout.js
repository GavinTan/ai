...
const { ref, reactive, toRefs, inject, computed, watch, h, nextTick, provide } = Vue;
const { useRouter, RouterLink, useRoute } = VueRouter;
const { darkTheme, createDiscreteApi, GlobalThemeOverrides } = naive;
...
export const LayoutComponent = {
    ...
    setup() {
        ...
        const getMenuList = () => {
            if (store.chatData) {
                Object.entries(store.chatData).reverse().forEach(i => {
                    state.menuList.push(
                        {
                            label: () => h(RouterLink, { class: 'text-white', to: { name: 'chat', params: { id: i[0] } } }, { default: () => i[1]['chatList'][0]?.content }),
                            key: i[0],
                            icon: () => h('iconify-icon', { icon: 'mdi:message-outline', class: 'text-white', width: '1rem', height: '1rem' })
                        }
                    )
                });
            };
        ...
        const handleSendMsg = async () => {
            if (store.sendMsg && !store.requesting) {
                if (!store.cid) {
                    const cid = uuidv4();
                    store.chatData[cid] = { chatList: [], model: store.model };
                    await router.push({ name: 'chat', params: { id: cid } });
                };
        ...
        const handleRegen = () => {
            store.runing = -1;
            store.chatData[store.cid].chatList[store.chatData[store.cid].chatList.length - 1].content = '';
            clidFetchData();
        };
        ...
        watch(() => store.chatData, (val) => {
            state.menuList = [];
            Object.entries(val).reverse().forEach(i => {
                state.menuList.push(
                    {
                        label: () => h(RouterLink, {
                            class: 'text-white flex items-center cursor-pointer break-all',
                            to: { name: 'chat', params: { id: i[0] } }
                        }, {
                            default: () => [
                                h('div', { class: 'flex-1 overflow-hidden break-all relative' }, [
                                    i[1]['chatList'][0]?.content,
                                    h('div', { class: 'absolute inset-y-0 right-0 w-8 z-10 masking' })
                                ]),
                                h('div', { class: 'btn-tools absolute flex right-1 z-10 text-gray-300' }, [
                                    // h('button', { class: 'p-1 hover:text-white', innerHTML: '<svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg>' }),
                                    h('button', { onClick: (e) => {
                                        e.preventDefault();
                                        e.stopPropagation();
